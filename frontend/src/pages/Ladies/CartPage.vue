...
<template>
  <section class="space-y-6">
    <div class="border border-stone-200 bg-white p-8">
      ...
      <h1 class="mt-3 text-3xl text-stone-900">Your Cart</h1>
    </div>

    <div v-if="cartStore.loading && !cartStore.loaded" class="border border-stone-200 bg-white p-8 text-center text-stone-500">
      Loading cart...
    </div>

    <div v-else-if="cartStore.isEmpty" class="border border-stone-200 bg-white p-12 text-center">
      <p class="text-2xl">🛒</p>
      <p class="mt-3 text-stone-600">Your cart is empty.</p>
      ...
      <div class="grid gap-6 lg:grid-cols-[1fr_320px]">
        <div class="space-y-3">
          <div
            ...
            :key="item.id"
            class="flex gap-4 border border-stone-200 bg-white p-4"
          >
            <img
            ...
            <div class="flex flex-1 flex-col justify-between">
              <div>
                ...
                <p v-if="item.product.sku" class="text-xs text-stone-400">{{ item.product.sku }}</p>
              </div>
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  <button
                    @click="decrement(item.id, item.quantity)"
                    class="flex h-7 w-7 items-center justify-center border border-stone-300 text-stone-700 hover:bg-stone-50"
                    :disabled="cartStore.loading"
                  >−</button>
                  <span class="w-8 text-center text-sm font-medium">{{ item.quantity }}</span>
                  <button
                    @click="increment(item.id, item.quantity)"
                    class="flex h-7 w-7 items-center justify-center border border-stone-300 text-stone-700 hover:bg-stone-50 disabled:opacity-40 disabled:cursor-not-allowed"
        ...
        <div class="border border-stone-200 bg-white p-6 self-start space-y-4">
          ...
          <router-link
            to="/checkout"
            class="block w-full bg-stone-900 py-3 text-center text-sm font-semibold text-white transition hover:bg-stone-700"
          >
            Proceed to Checkout
          </router-link>
          <router-link to="/store" class="block w-full border border-stone-300 py-3 text-center text-sm font-semibold text-stone-900 transition hover:bg-stone-50">
            Continue Shopping
          </router-link>
        </div>
      </div>

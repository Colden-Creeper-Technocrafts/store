...
async function shareInstagram() {
  // Sharing an image FILE opens Instagram's post composer.
  // Sharing just a URL only opens Instagram DMs — which is not what the user wants.
  const imgUrl = displayImage.value
  const hasImage = imgUrl && !imgUrl.includes('placeholder')

  if (navigator.share && hasImage) {
    try {
      ...
      // CORS not set on image server — fall through to URL share
    }
  }
